...
export default function DatabasePage() {
    ...
    const exportTableData = (tableName: string) => {
        ...
        const exportFileDefaultName = `${tableName}_backup_${new Date().toISOString().split('T')[0]}.json`;
    ...
    const currentTableData = selectedTable ? databaseInfo.tableData[selectedTable] : null;
    const filteredData = currentTableData?.data.filter(row => 
        JSON.stringify(row).toLowerCase().includes(searchTerm.toLowerCase())
    ) || [];

    // Calculate total stats
    const totalTables = databaseInfo.tables.length;
    const totalRows = Object.values(databaseInfo.tableData).reduce((sum, table) => sum + table.rowCount, 0);
    const totalColumns = Object.values(databaseInfo.tableData).reduce((sum, table) => sum + table.columnCount, 0);
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500 mt-2">Complete database overview and management</p>
                </div>
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-100">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900 mt-1">{totalTables}</p>
                        </div>
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-100">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900 mt-1">{totalRows.toLocaleString()}</p>
                        </div>
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-100">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-gray-900 mt-1">{totalColumns}</p>
                        </div>
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-100">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-sm font-semibold text-green-600 mt-1">PostgreSQL + PostGIS</p>
                        </div>
                        <div className="p-3 bg-green-100 rounded-lg">
                            <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div>
                                                ...
                                                <p className="text-xs text-gray-500 mt-1">
                                                    {tableData.rowCount} rows • {tableData.columnCount} cols
                                                </p>
                                            </div>
                                            {isSelected && (
                                            ...
                                            {currentTableData.columns.map((column) => (
                                                <tr key={column.column_name} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        <code className="bg-gray-100 px-2 py-1 rounded">{column.data_type}</code>
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        {column.column_default ? (
                                                            <code className="bg-gray-100 px-2 py-1 rounded text-xs">{column.column_default}</code>
                                                        ) : (
                                    ...
                                    {filteredData.length > 0 ? (
                                                                ...
                                                                {row[column.column_name] !== null ? (
                                                                    typeof row[column.column_name] === 'object' ? (
                                                                        <code className="bg-gray-100 px-2 py-1 rounded text-xs">
                                                                            {JSON.stringify(row[column.column_name]).slice(0, 50)}...
                                                                        </code>
                                                                    ) : (
                                                                        <span className="max-w-xs truncate block" title={String(row[column.column_name])}>
                                        ...
                                        <div className="text-center py-12">
                                            <p className="text-gray-500">
                                                {searchTerm ? 'No matching data found' : 'No data in this table'}
                                            </p>
                                        </div>
                                    )}
